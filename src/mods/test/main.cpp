...
namespace mods::test
{
	...
	BOOL CALLBACK enum_windows_proc(HWND hwnd, LPARAM lParam)
	{
		...
		if (window_pid == target_pid && IsWindowVisible(hwnd))
		{
			...
			if (std::string_view(class_name).contains("ZSystemClass"s))
			{
...
//	}
	...
	DWORD WINAPI find_game_window_by_sha1([[maybe_unused]] LPVOID lpParam)
	{
		...
		std::cout << "[MAIN] Waiting for window with classname containing 'ZSystemClass' ... \n";

		{
			while (!shared::globals::main_window)
			{
				...
				if (T >= 30000)
				{
					...
					shared::common::console(); std::cout << "[MAIN] Could not find ZSystemClass Window. Not loading RTX Compatibility Mod.\n";
					return TRUE;
				}
			}
		}
...
BOOL APIENTRY DllMain(HMODULE hmodule, const DWORD ul_reason_for_call, LPVOID)
{
	if (ul_reason_for_call == DLL_PROCESS_ATTACH) 
	{
		...
		mods::test::game::exe_module = GetModuleHandleA(nullptr);
		mods::test::install_signature_patches();

		if (const auto t = CreateThread(nullptr, 0, mods::test::find_game_window_by_sha1, nullptr, 0, nullptr); t) {
			CloseHandle(t);
		}
	}
