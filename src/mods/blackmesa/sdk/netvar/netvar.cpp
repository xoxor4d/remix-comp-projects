...
namespace sdk
{
									...
									std::map< uintptr_t, std::string > >& var_dump, const size_t child_offset)
	{
		for (auto i = 0; i < table->props_count; ++i) 
		{
			...
			const auto name_hash = shared::utils::fnv::hash(prop.prop_name);
	...
	static void initialize_props(netvar_table_map& table_map)
	{
		...
		for (auto client_class = mods::blackmesa::interfaces::get()->m_client->get_client_class(); client_class; client_class = client_class->next_ptr) 
		{
			const auto table = client_class->recvtable_ptr;
			const auto table_name = table->table_name;
			const auto table_name_hash = shared::utils::fnv::hash(table_name);
