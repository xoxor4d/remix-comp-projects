...
namespace mods::gh3
{
	...
	BOOL CALLBACK enum_windows_proc(HWND hwnd, LPARAM lParam)
	{
		...
		if (window_pid == target_pid && IsWindowVisible(hwnd))
		{
			...
			if (std::string_view(class_name).contains("AspyrGH3"s))
			{
...
//	}
	...
	DWORD WINAPI find_game_window_by_sha1([[maybe_unused]] LPVOID lpParam)
	{
		...
		std::cout << "[MAIN] Waiting for window with classname containing 'AspyrGH3' ... \n";

		{
			while (!shared::globals::main_window)
			{
				...
				if (T >= 30000)
				{
					...
					shared::common::console(); std::cout << "[MAIN] Could not find 'AspyrGH3' Window. Not loading RTX Compatibility Mod.\n";
					return TRUE;
				}
			}
		}
...
BOOL APIENTRY DllMain(HMODULE hmodule, const DWORD ul_reason_for_call, LPVOID)
{
	if (ul_reason_for_call == DLL_PROCESS_ATTACH) 
	{
		...
		shared::common::loader::module_loader::register_module(std::make_unique<mods::gh3::d3d9ex>());

		if (const auto t = CreateThread(nullptr, 0, mods::gh3::find_game_window_by_sha1, nullptr, 0, nullptr); t) {
			CloseHandle(t);
		}
	}
