	...
	void cont_general_quickcommands()
	{
...

		{
			...
			cont_cull_height = ImGui::Widget_ContainerWithCollapsingTitle("Camera", cont_cull_height, [&]
			{
				...
				ImGui::Checkbox("Texture Hack Disable Stage 0", &im->m_dbg_disable_stage0);
				if (ImGui::SliderInt("Texture FLAG Stage 0", &im->m_dbg_textureflags_index2, 0, 9)) {
					im->m_dbg_textureflags_index2 = std::clamp(im->m_dbg_textureflags_index2, 0, 9);
				}

				ImGui::Separator();

				SET_CHILD_WIDGET_WIDTH_MAN(140.0f);
				if (ImGui::SliderInt("Texture Stage", &im->m_dbg_texture_stage_index, 0, 50)) {
					im->m_dbg_texture_stage_index = std::clamp(im->m_dbg_texture_stage_index, 0, 50);
				}

				SET_CHILD_WIDGET_WIDTH_MAN(140.0f);
				if (ImGui::SliderInt("Texture FLAG", &im->m_dbg_textureflags_index, 0, 9)) {
					im->m_dbg_textureflags_index = std::clamp(im->m_dbg_textureflags_index, 0, 9);
				}
