...
#pragma region D3D9Device
						...
						start_register < constant_desc.RegisterIndex + constant_desc.RegisterCount)
					{
						...
						/*if (name == "$mProjection") 
						{
							if (Vector4fCount >= 4)
							{
								...
								dev->SetTransform(D3DTS_PROJECTION, &temp);
							}
	...
	{
		static uint32_t retn_addr = 0x100DD13C;
		__asm
		{
			pushad;
			push	eax;
	...
	HRESULT d3d9ex::D3D9Device::SetVertexShaderConstantF(UINT StartRegister, CONST float* pConstantData, UINT Vector4fCount)
	{
		...
		if (/*im->m_dbg_use_fake_camera &&*/ asd(m_pIDirect3DDevice9, StartRegister, pConstantData, Vector4fCount))
		{
			/*if (Vector4fCount >= 4)
			{
				D3DXMATRIX temp;
				memcpy(&temp, pConstantData, sizeof(D3DXMATRIX));
				m_pIDirect3DDevice9->SetTransform(D3DTS_WORLD, &temp);
			}*/
		}
...
#pragma endregion
	...
	d3d9ex::d3d9ex()
	{
		...
		const auto d3d9_import_addr = shared::utils::mem::find_import_addr(shared::globals::exe_hmodule, "d3d9.dll", "Direct3DCreate9");
		if (d3d9_import_addr) 
		{
		...
		// 100DD137
		shared::utils::hook::nop(0x100DD134, 3);
		shared::utils::hook(0x100DD137, on_mvp_calc_stub, HOOK_JUMP).install()->quick();
	}
}
