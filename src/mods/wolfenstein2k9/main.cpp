...
namespace mods::wolfenstein2k9
{
	...
	BOOL CALLBACK enum_windows_proc(HWND hwnd, LPARAM lParam)
	{
		...
		if (window_pid == target_pid && IsWindowVisible(hwnd))
		{
			...
			if (std::string_view(class_name) == "Wolf2"s) // contains fails because of the splash
			{
...
//	}
	...
	DWORD WINAPI find_game_window_by_sha1([[maybe_unused]] LPVOID lpParam)
	{
		...
		shared::common::loader::module_loader::register_module(std::make_unique<mods::wolfenstein2k9::d3d9ex>());

		std::cout << "[MAIN] Waiting for window with classname containing 'Wolf2' ... \n";

		{
			while (!shared::globals::main_window)
			{
				...
				if (T >= 30000)
				{
					...
					shared::common::console(); std::cout << "[MAIN] Could not find Wolf2 Window. Not loading RTX Compatibility Mod.\n";
					return TRUE;
				}
			}
		}
