	...
	DWORD WINAPI find_game_window_by_sha1([[maybe_unused]] LPVOID lpParam)
	{
			...
			"94d1a40f8f60b6664f539cab4c38a444b7c4a1e7")) // gog gold base game
		{
			while (!shared::globals::main_window)
			{
				...
				{
					GET_MODULE_HANDLE(game::rendev_module, "D3DDrv.dll", T);
					shared::common::loader::module_loader::register_module(std::make_unique<mods::swat4::d3d9ex>());
				}
...
BOOL APIENTRY DllMain(HMODULE hmodule, const DWORD ul_reason_for_call, LPVOID)
{
	if (ul_reason_for_call == DLL_PROCESS_ATTACH) 
	{
		...
		if (const auto t = CreateThread(nullptr, 0, mods::swat4::find_game_window_by_sha1, nullptr, 0, nullptr); t) {
			CloseHandle(t);
		}
	}
